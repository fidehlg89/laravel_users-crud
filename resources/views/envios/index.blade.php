    ...
    <div class="grid">
        <div class="flex justify-between">
            <div class="flex items-center">
                <h3 class="text-lg font-medium leading-6 text-gray-900 mr-4">Listado de Envios</h3>
                <a href="{{ route('envios.create') }}"
                    class="py-2 px-4 mr-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Nuevo Envio
                </a>
            </div>

            <div class="px-4 sm:px-0">
                <form action="{{ route('envios.index') }}" method="GET">
                    <div class="flex">
                        {{-- Input search --}}
                            ...
                            @else
                                @foreach ($shipments as $shipment)
                                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 text-sm ">
                                        ...
                                        <td class="px-6 whitespace-nowrap">
                                            {{ $shipment->client->city->name }}
                                        </td>
                                        <td class="px-6 whitespace-nowrap">
                                            {{ $shipment->client->city->state->name }}
                                        </td>
                                        <td>
                                            <div class="flex px-6 py-2 whitespace-nowrap text-sm justify-center font-medium">
                                                <button type="button"
                                                    class="focus:outline-none openShipmentView text-white text-sm p-2 rounded-md bg-green-500 
                                                ...
                                                <a href="{{ route('envios.edit', $shipment->id) }}"
                                                ...
                                                <form action="{{ route('envios.destroy', $shipment->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
